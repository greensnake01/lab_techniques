...
The package *rstatix* serves for Dunn's post-hoc test after Kruskal-Wallis.
...
The package *dunn.test* serves for Dunn's post-hoc test after Kruskal-Wallis.
...
```{r 1st}
data <- read.csv('../../Data/latest_EcoLab 2025 Mastersheet.xlsx - Mastersheet.csv', sep = ',')
```
...
```{r}
data_clean <- data[, 1:(length(data))] # create a copy with all necessary columns

data_clean[3,51] <- "Peptidase_activityF0"
data_clean[3,52] <- "Peptidase_activityF1"
data_clean[3,53] <- "Peptidase_activityF3"
data_clean[3,54] <- "Peptidase_activityF5"

colnames(data_clean) <- data_clean[3, 1:(length(data_clean))] # use the values in the 3rd row as the column names
...
# change variable name for DIP/TP to Olsen_P_[DIP]
colnames(data_clean)[36] <- 'Olsen_P_[DIP]/TP'

# correct negative values of Cmic and Nmic
data_clean$Cmic[data_clean$Cmic < 0] <- NA
data_clean$Nmic[data_clean$Nmic < 0] <- NA

# add enzymes/Cmic
#data_clean$`Cellulase/Cmic` <- data_clean$Cellulase / data_clean$Cmic
#data_clean$`Chitinase/Cmic` <- data_clean$Chitinase / data_clean$Cmic
#data_clean$`Phosphatase/Cmic` <- data_clean$Phosphatase / data_clean$Cmic

# add P pool
#data_clean$`Olsen_P_[DIP]/TPa` <- (data_clean$`Olsen_P_[DIP]` / data_clean$Soil_TP) * 100
data_clean$`TIP/TP` <- (data_clean$Soil_TIP / data_clean$Soil_TP) * 100

# add enzymes/Pmic
data_clean$`Cellulase/Pmic` <- data_clean$Cellulase / data_clean$Pmic
data_clean$`Chitinase/Pmic` <- data_clean$Chitinase / data_clean$Pmic
data_clean$`Phosphatase/Pmic` <- data_clean$Phosphatase / data_clean$Pmic

# add enzymes/appropriate C- or N-mic
data_clean$`Cellulase/Cmic` <- data_clean$Cellulase / data_clean$Cmic
data_clean$`Chitinase/Cmic` <- data_clean$Chitinase / data_clean$Cmic
data_clean$`Chitinase/Nmic` <- data_clean$Chitinase / data_clean$Nmic

# try N:P ratio - soil? mic?
data_clean$`Soil_C/N` <- round(data_clean$Soil_C/data_clean$Soil_N, 2)
colnames(data_clean)[10] <- 'Soil_C:N'
data_clean$`Microbial_C:N` <- round(data_clean$Cmic/data_clean$Nmic, 2)

# try C:P
data_clean$`Microbial_C:P` <- round(data_clean$Cmic/data_clean$Pmic, 2)

data_clean$`C:P(TOP)` <- round(data_clean$Soil_C/data_clean$Soil_TOP, 2)

str(data_clean) # print the value types
```

```{r}
units <- data[4, 1:length(data)] # add the units into the last row of an object storing units

colnames(units) <- colnames(data_clean[c(1:55)-1])

#colnames(units) <- data_clean[3, 1:(length(data_clean)-2)] # use the values in the 3rd row as the column names
...
#add P pool
units$`Olsen_P_[DIP]/TP` <- "%"
units$`TIP/TP` <- "%"

units$`Cmic` <- "[μg C g-1 DW]"
units$`Nmic` <- "[μg N g-1 DW]"
units$Pmic <- "[µg P g-1 DW]"
units$Cellulase <- "[µmol g-1 DW h-1]"  
units$Chitinase <- "[µmol g-1 DW h-1]"
units$Phosphatase <- "[µmol g-1 DW h-1]"
units$Protease <- "[µmol g-1 DW h-1]"
units$`Cellulase/Pmic` <- "[µmol g-1 DW h-1 / µg P g-1 DW]" # fix unit issue
units$`Chitinase/Pmic` <- "[µmol g-1 DW h-1 / µg P g-1 DW]" # fix unit issue
units$`Phosphatase/Pmic` <- "[µmol g-1 DW h-1 / µg P g-1 DW]" # fix unit issue

units$`Cellulase/Cmic` <- "[µmol g-1 DW h-1 / µg C g-1 DW]"
units$`Chitinase/Cmic` <- "[µmol g-1 DW h-1 / µg C g-1 DW]"
units$`Chitinase/Nmic` <- "[µmol g-1 DW h-1 / µg N g-1 DW]"
...
# 6 *Square Root Transformed Data*

Square root transformation of data to achieve normality for statistical tests.

```{r}
data_clean_sqrt <- data_clean
...
dataframe = data_clean_log # data_clean or data_clean_sqrt or data_clean_log

# interval comprising column numbers we would like to use
from = 7
to = 59
...
for (variableX in c(27:29,31:38,55:62)){ # from cellulase through TOP/TP to Cellulase, Chitinase and Phosphatase / Pmic and C-/N-mic
...
## 9.1 *Automatic Plotting*

```{r}
box_plots <- list()
for (variable in c(10,27:29,31:38,55:63,66)){ # all 7:length(variables)
...
bar <- ggplot(mean, aes(.data[[variables[x]]], 
  ...
  scale_fill_manual(values = c(col1, col2, col3))+
    ...
    labs(title = paste(variables[y], ' x ', variables[x], "-", "SD"),
...
bar_sem <- ggplot(mean, aes(.data[[variables[x]]], 
  ...
  scale_fill_manual(values = c(col1, col2, col3))+
    ...
    labs(title = paste(gsub('_', ' ', variables[y]), ' x ', variables[x], "-", "SEM"),
         y = paste(gsub('_', ' ', variables[y]), ' ', units[1,variables[y]]),
         x = paste(variables[x]),
         fill = 'Legend')+
...
for (variable in 7:length(variables)){
  ...
  barplot_sd <- ggplot(summary_data, aes(.data[[variables[x]]], 
    ...
    scale_fill_manual(values = c(col1, col2, col3))+
      ...
      labs(title = paste(gsub('_', ' ', variables[variable]), ' x ', variables[x], "-", "SD"),
           y = paste(gsub('_', ' ', variables[variable]), ' ', units[1, variables[variable]]),
           x = paste(variables[x]),
           fill = 'Legend')+
  ...
  geom_errorbar(aes(ymin = .data[[variables[variable]]] - .data[[paste0(variables[variable], "_sem")]],
                ...
                size  = 0.7) +
  scale_fill_manual(values = c(col1, col2, col3)) +
  labs(title = paste(gsub('_', ' ', variables[variable]), '×', variables[x], "-", "SEM"),
       y     = paste(gsub('_', ' ', variables[variable]), units[1, variables[variable]]),
       x     = variables[x],
       fill  = 'Legend') +
  theme_bw() +
...
# 12 *Statistics with Groups*

## 12.1 *Meet Assumptions of ANOVA*
...
for (variable in 7:length(variables)){
  ...
  if (is.na(dataframe[1 , variables[variable]])){
    next
  }
  
  print(variables[variable])
  
  shapiro <- shapiro.test(dataframe[[variables[variable]]])
...
### 12.1.2 *Homogeneity of Variance Test*
...
for (variable in 7:length(variables)){
  ...
  levene <- leveneTest(dataframe[[variables[variable]]] ~ data_clean[[x]])
...
for (variable in 7:length(variables)){
  ...
  anova <- aov(dataframe[[variables[variable]]] ~ dataframe[[variables[x]]])
...
for (variable in 1:length(anova_summary)){
  ...
  print(paste("Tukey's HSD", 'performed for:', anova_model[variable]))
...
Kruskal-Wallis test is a non-parametric method for testing whether samples originate from the same distribution. It is used when the assumptions of ANOVA are not met to find the differences between groups.

```{r}
# example by hand
#kruskal_wallis <- kruskal_test(Pmic ~ Depth, data = dataframe)
#kruskal_wallis

kruskal_wallis_summary <- list()
...
for (variable in 7:length(variables)){
  ...
  kruskal_wallis <- kruskal_test(dataframe, dataframe[[variables[variable]]] ~ dataframe[[variables[x]]])

  kruskal_wallis_summary[[variables[variable]]] <- kruskal_wallis
  
  print(paste('Kruskal-Wallis performed for:', variables[variable]))
}
```
...
```{r}
print(paste('Variable:', variables[y]))
print(kruskal_wallis_summary[[variables[y]]]["p"])
```

## 12.5 *Dunn's Test*

Dunn's test is a non-parametric post-hoc test used after the Kruskal-Wallis test to identify which specific groups differ from each other.
...
kruskal_wallis_variables <- names(kruskal_wallis_summary)
dunn_summary <- list()

for (variable in 1:length(kruskal_wallis_variables)){

  dunn <- dunn.test(dataframe[[kruskal_wallis_variables[variable]]],
                    dataframe[[variables[x]]],
                    method = "bonferroni")

  dunn_summary[[kruskal_wallis_variables[variable]]] <- dunn
  
  print(paste("Dunn's Test", 'performed for:', kruskal_wallis_variables[variable]))
...
```{r}
print(paste('Variable:', variables[y]))
print(dunn_summary[[variables[y]]][c("P.adjusted", "comparisons")])
```
...
for (variable in names(anova_summary)){
  export_data <- rbind(export_data, 
                       data.frame(variable = variable,
                                  ...
                                  tukey_p = tukey_summary[[variable]][["dataframe[[variables[x]]]"]][,"p adj"],
                                  kruskal_wallis_p = kruskal_wallis_summary[[variable]][["p"]],
                                  dunn_p = dunn_summary[[variable]][["P.adjusted"]]))
}

rownames(export_data) <- 1:nrow(export_data)
export_data$tukey_or_dunn_groups <- rep(c("B-A", "C-A", "C-B"), times = nrow(export_data)/3)
```
...
export_data_excel <- write.xlsx(export_data, 
                               file = paste0(path, "Data/log_stats_results_C_P.xlsx"), 
                               sheetName = "stat_test", 
                               rowNames = F,
                               overwrite = T)
```

## ---12.4 *Special Case - PERMANOVA - not used, skip*
...
correlations_gen <- ggpairs(data_clean, 
                        columns = c(27:29,31:38), # from cellulase to TOP/TP # 55:57 Phosphatase/Pmic - all correlate
...
correlations_layer <- ggpairs(data_clean, 
                        columns = c(27:29,31:38), # from cellulase to TOP/TP
                        title = "Correlations P-Group", 
                        upper = list(continuous = wrap("cor", size = 2.5)),
                        lower = list(continuous = "smooth"),
                        aes(colour = Depth))+
...
ols_gen <- ggplot(data_clean, aes(x = data_clean[[z]], 
  ...
  scale_shape_manual(values = c("A" = 16, "B" = 17, "C" = 15))+
      ...
      labs(title = paste(gsub('_', ' ', variables[y]), ' x ', gsub('_', ' ', variables[z])),
           y = paste(gsub('_', ' ', variables[y]), ' ', units[1, variables[y]]),
           x = paste(gsub('_', ' ', variables[z]), ' ', units[1, variables[z]]),
           colour = 'Legend',
           shape = 'Legend')+
...
ols_layer <- ggplot(data_clean, aes(x = data_clean[[z]], 
  ...
  scale_shape_manual(values = c("A" = 16, "B" = 17, "C" = 15))+
      ...
      labs(title = paste(gsub('_', ' ', variables[z]), ' x ', gsub('_', ' ', variables[z])),
           y = paste(gsub('_', ' ', variables[y]), ' ', units[1, variables[y]]),
           x = paste(gsub('_', ' ', variables[z]), ' ', units[1, variables[z]]),
           colour = 'Legend',
           shape = 'Legend',
...
### 14.1-2.1 *Automated Plotting !!!per Y variable!!!*
...
### 14.1-2.2 *Automated Plotting from chosen variable to chosen variable in a range*

```{r}
ols_plots_gen <- list()
ols_plots_layers <- list()

for (variableX in c(27:29,31:38,55:57)){ # all 7:length(variables)

  for (variableY in c(27:29,31:38,55:57)){ # all 7:length(variables)

    if (is.na(data_clean[1 , variables[variableX]]) | is.na(data_clean[1 , variables[variableY]] | variableX == variableY)){
      next
    }

    ols_gen <- ggplot(data_clean, aes(x = .data[[variables[variableX]]],
                                      y = .data[[variables[variableY]]]))+
    ...
    scale_shape_manual(values = c("A" = 16, "B" = 17, "C" = 15))+
        ...
        labs(title = paste(gsub('_', ' ', variables[variableY]), ' x ', gsub('_', ' ', variables[variableX])),
             y = paste(gsub('_', ' ', variables[variableY]), ' ', units[1, variables[variableY]]),
             x = paste(gsub('_', ' ', variables[variableX]), ' ', units[1, variables[variableX]]))+
    ...
    ols_plots_gen[[paste(variables[variableY], "_vs_", variables[variableX], sep = "")]] <- ols_gen

    ols_layers <- ggplot(data_clean, aes(x = .data[[variables[variableX]]],
                                         y = .data[[variables[variableY]]],
                                         colour = .data[[variables[x]]],
                                         fill = .data[[variables[x]]]))+
  ...
  scale_shape_manual(values = c("A" = 16, "B" = 17, "C" = 15))+
    ...
    ols_plots_layers[[paste(variables[variableY], "_vs_", variables[variableX], sep = "")]] <- ols_layers
  }
}
```
...
## 15.2 *Special Cases Multiple Predictors*
...
Layer-specific OLS regression can be performed to account for differences between layers A, B, and C.

```{r}
# example by hand
ols_stat_layers <- dataframe %>%
  group_by(.data[[variables[x]]]) %>%
  do(model = lm(.data[[variables[y]]] ~ .data[[variables[z]]], data = .))

for (layer in 1:length(ols_stat_layers$model)){
  print(paste('Layer:', ols_stat_layers[[variables[x]]][layer]))
  print(summary(ols_stat_layers$model[[layer]]))
}
```

```{r}
# manual confirmation of calculation for correct groups
test_df_A <- data_clean %>%
  filter(Depth == 'A')

lm_A <- lm(test_df_A[[variables[y]]] ~ test_df_A[[variables[z]]])
print("Layer A")
summary(lm_A)


test_df_B <- data_clean %>%
  filter(Depth == 'B')

lm_B <- lm(test_df_B[[variables[y]]] ~ test_df_B[[variables[z]]])
print("Layer B")
summary(lm_B)


test_df_C <- data_clean %>%
  filter(Depth == 'C')

lm_C <- lm(test_df_C[[variables[y]]] ~ test_df_C[[variables[z]]])
print("Layer C")
summary(lm_C)
```

Automated calculation for all variables.

```{r}
ols_stat_layers_summary <- list()
...
for (variableX in 7:length(variables)){
  ...
  for (variableY in 7:length(variables)){
    ...
    ols_stat_layers <- data_clean %>%
      group_by(.data[[variables[x]]]) %>%
      do(model = lm(.data[[variables[variableY]]] ~ .data[[variables[variableX]]], data = .))
    
    ols_stat_layers_summary[[paste(variables[variableY], "_vs_", variables[variableX], sep = "")]] <- list()
    
    for (layer in 1:length(ols_stat_layers$model)){
      ols_stat_layers_summary[[paste(variables[variableY], "_vs_", variables[variableX], sep = "")]][[ols_stat_layers[[variables[x]]][layer]]] <- summary(ols_stat_layers$model[[layer]])
    }
    
  }
  print(paste('Layer-specific OLS Regression performed for:', variables[variableX]))
}
```

## 15.3.1 *Saving*

Export results into a separate dataframe.

```{r}
export_data <- data.frame()

for (combination in names(ols_stat_layers_summary)){
  
  for (layer in seq_along(ols_stat_layers_summary[[combination]])){
    export_data <- rbind(export_data,
                         data.frame(variableY_vs_variableX = combination,
                                    ols_p = ols_stat_layers_summary[[combination]][[layer]][["coefficients"]][2,"Pr(>|t|)"],
                                    r_adjusted = ols_stat_layers_summary[[combination]][[layer]][["adj.r.squared"]]))
  }
}

export_data$layer <- rep(c("A", "B", "C"), times = nrow(export_data)/3)
```
...
```{r}
export_data_excel <- write.xlsx(export_data, 
                               file = paste0(path, "Data/ols_layers_results.xlsx"), 
                               sheetName = "ols_layers", 
                               rowNames = F,
                               overwrite = T)
```

# ---16 *LMM Regression - not used, skip*

Linear Mixed Models (LMM) are an extension of linear models that allow for the inclusion of both fixed and random effects. They are particularly useful when dealing with hierarchical or grouped data, where observations within the same group may be correlated. ct(lmm, terms = c(data_clean[[w]], data_clean[[w]])))
...
```{r}
ggplot(data_clean, aes(x = Depth, y = `Phosphatase/Pmic`)) +
  #geom_col() +
  geom_boxplot()+
  labs(x = "Depth",
       y = "Phosphatase/Pmic",
       title = "Phosphatase/Pmic by Depth") +
  theme_minimal()
```
```{r}
ggplot(data_clean, aes(x = `Olsen_P_[DIP]`, y = `Phosphatase/Pmic`)) +
  geom_point(aes(colour = Depth)) +
  geom_smooth(method = "lm") +
  labs(x = "Olsen_P_[DIP]",
       y = "Phosphatase/Pmic",
       title = "Phosphatase/Pmic x Olsen_P_[DIP]") +
  theme_minimal()
```

```{r}
linear_model <- lm(`Phosphatase/Pmic` ~ `Olsen_P_[DIP]`, data_clean)
summary(linear_model)
```
```{r}
data_clean$Depth_test <- factor(data_clean$Depth, levels = c("A", "B", "C"))
data_clean$Depth_numeric <- as.numeric(data_clean$Depth_test)

ggplot(data_clean, aes(x = Depth_numeric, y = Soil_TIP))+
  geom_point()+
  geom_smooth(method = "lm")
  
test_lm <- lm(Depth_numeric ~ Soil_TIP, data_clean) 
summary(test_lm)

test_lm_npmic <- lm(Nmic ~ Pmic, data_clean) 
summary(test_lm_npmic)
```
