...
# correct negative values of Cmic
#data_clean$Cmic[data_clean$Cmic < 0] <- 10
...
units$`Cellulase/Pmic` <- "[µmol g-1 DW h-1 / µg g-1 DW]" # fix unit issue
units$`Chitinase/Pmic` <- "[µmol g-1 DW h-1 / µg g-1 DW]" # fix unit issue
units$`Phosphatase/Pmic` <- "[µmol g-1 DW h-1 / µg g-1 DW]" # fix unit issue
...
dataframe = data_clean # data_clean or data_clean
...
for (variableX in c(27:29,31:38,55:57)){ # from cellulase through TOP/TP to Cellulase, Chitinase and Phosphatase / Pmic
...
for (variable in names(anova_summary)){
  export_data <- rbind(export_data, 
                       data.frame(variable = variable,
                                  ...
                                  kruskall_wallis_p = kruskal_wallis_summary[[variable]][["p"]],
...
export_data_excel <- write.xlsx(export_data, 
                               file = paste0(path, "Data/stats_results.xlsx"), 
                               sheetName = "stat_test", 
                               rowNames = F,
                               overwrite = T)
```
