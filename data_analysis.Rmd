...
```{r}
x = 35 # 4 for Depth, 35 for Soil_TP, 38 for Pmic
y = 38
fill = 4
...
## ---9.4 *Special Case - PERMANOVA - not used*
...
```{r}
ggplot(data_clean, aes(x = data_clean[[z]], 
                       y = data_clean[[y]]))+
...
# loop through all variables against Depth
for (variableX in 5:length(variables)){
  ...
  for (variableY in 5:length(variables)){
    # skip columns with NAs
...
```{r}
print(paste('Variable Y:', variables[y], 'Variable X:', variables[x]))
print(ols_summary[[variables[x]]][[variables[y]]])
```

# ---13 *Special Cases (Multiple Predictors)*

Multiple predictors can be included in the model to account for additional sources of variation.

# ---14 *LMM Regression - not used*
...
# ---15 *Automated Plotting and Saving*
