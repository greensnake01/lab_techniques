...
#add P pool
data_clean$`Olsen_P_[DIP]/TP` <- (data_clean$`Olsen_P_[DIP]` / data_clean$Soil_TP) * 1100
...
```{r}
box_plots <- list()
for (variable in c(27:29,31:38,55:62)){ # all 7:length(variables)
...
export_data_excel <- write.xlsx(export_data, 
                               file = paste0(path, "Data/sqrt_stats_results_mics.xlsx"), 
                               sheetName = "stat_test", 
                               rowNames = F,
                               overwrite = T)
```
